...
for entry in feed.entries:
    ...
    file_name = file_name.replace(':', '-')  # 콜론을 대시로 대체 (윈도우에서 파일명에 사용 안됨)
    file_name = file_name.replace('*', '-')  # 별표를 대시로 대체
    file_name = file_name.replace('?', '-')  # 물음표를 대시로 대체
    file_name = file_name.replace('"', '-')  # 큰따옴표를 대시로 대체
    file_name = file_name.replace('<', '-')  # <를 대시로 대체
    file_name = file_name.replace('>', '-')  # >를 대시로 대체
    file_name = file_name.replace('|', '-')  # |를 대시로 대체
    file_name += '.md'
    ...
    try:
        ...
        repo.git.commit('-m', f'Add or update post: {entry.title}')
